class SiteMaker { // GitHub Pages で稼働するようファイル作成する（git push するファイルを作る）
    ...
    async make() { // 初回にリモートリポジトリを作成するとき一緒に作成する
        ...
        await Promise.all([
            ...
            window.myApi.cp(`src/lib/toastify/1.11.2/min.css`, `dst/${this.setting.github.repo}/lib/toastify/1.11.2/min.css`, {'recursive':true, 'preserveTimestamps':true}),
            window.myApi.cp(`src/lib/toastify/1.11.2/min.js`, `dst/${this.setting.github.repo}/lib/toastify/1.11.2/min.js`, {'recursive':true, 'preserveTimestamps':true}),
